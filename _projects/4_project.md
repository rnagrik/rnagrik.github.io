...
We start by defining an MDP with a discrete state space $X$ capturing position, orientation, key possession, and door state, a control space $U$, and a motion model $f$. We define transition costs for valid transitions. Running **backward DP** over a finite horizon yields the value function and the optimal action at each state.
